        ...
        switch (def.name) {
          case 'set_goal': {
            ...
            if (action === 'create') {
              ...
              result._suggestions = [
                {
                  tool: 'evaluate_situation',
                  args: {
                    action: 'quick',
                    quick_context: `Evaluating situation for goal ${result.title}`,
                  },
                  reason: 'Evaluate situation and candidate actions for new goal',
                },
              ];
            } else if (action === 'update') {
              result = GoalEngine.updateGoal(db, { project, ...args });
            } else if (action === 'decompose') {
              result = GoalEngine.decomposeGoal(db, {
                project,
                parent_id: args.id || args.parent_id,
                subgoals: args.subgoals || [],
              });
            } else if (action === 'get') {
              result = GoalEngine.getGoal(db, { project, id: args.id });
            } else if (action === 'list') {
              result = GoalEngine.listGoals(db, {
                project,
                status: args.status,
              ...
              result = GoalEngine.abandonGoal(db, {
                project,
                id: args.id,
          ...
          case 'evaluate_situation': {
            ...
            result._suggestions = [
              {
                tool: 'manage_intentions',
                args: {
                  action: 'create',
                  goal_id: result.chosen_action?.action,
                  trace_id: result.trace_id,
                  behavior_name: result.chosen_action?.action,
                },
                reason: 'Create intention for behavior runtime execution',
              },
              {
                tool: 'get_decision_trace',
                args: { action: 'get', trace_id: result.trace_id },
                reason: 'Inspect explainable chain-of-thought rationale',
              },
            ];
            break;
          }
            ...
            result._suggestions = [
              {
                ...
                args: {
                  action: 'quick',
                  quick_context: `Post-replan situation check for goal ${args.goal_id}`,
                },
                reason: 'Re-evaluate situation following replan',
              },
            ];
            break;
          }
          ...
          case 'assess_risk': {
            if (args.action === 'compare' && args.candidate_actions) {
              ...
              result = RiskEngine.assessAction(
                args.candidate_action || 'default_action',
                args.parameters,
                args.situation_context
              );
            }
            break;
          }
          ...
          case 'set_utility_weights': {
            ...
            if (action === 'configure') {
              ...
              result = args.name
                ...
                : UtilityProfileEngine.getActiveProfile(db, project);
            } else if (action === 'list') {
          ...
          case 'get_decision_trace': {
            const action = args.action;
            if (action === 'latest') {
              result = DecisionTraceEngine.getLatestTrace(db, project) || {
                message: 'No decision traces found.',
              };
            } else if (action === 'get') {
              result = DecisionTraceEngine.getTrace(db, { project, id: args.trace_id });
            } else if (action === 'list') {
              result = DecisionTraceEngine.listTraces(db, {
                project,
          ...
          case 'manage_intentions': {
            ...
            if (action === 'create') {
              ...
              result._suggestions = [
                {
                  tool: 'behavior-mcp:load_behavior',
                  args: {
                    action: 'load',
                    behavior_name: result.behavior_name,
                    intention_id: result.id,
                  },
                  reason: 'Execute loaded intention in behavior runtime',
                },
              ];
              ...
              result = IntentionEngine.listIntentions(db, {
                project,
                status: args.status,
                goal_id: args.goal_id,
              });
            } else if (action === 'cancel') {
              result = IntentionEngine.resolveIntention(db, {
                project,
                id: args.intention_id,
                status: 'aborted',
                result: args.result,
              });
          ...
          case 'manage_reasoning_db': {
            ...
            if (action === 'stats') {
              const goalsCount = (
                db.prepare('SELECT COUNT(*) as c FROM goals WHERE project = ?').get(project) as any
              ).c;
              const beliefsCount = (
                db
                  .prepare('SELECT COUNT(*) as c FROM beliefs WHERE project = ?')
                  .get(project) as any
              ).c;
              const tracesCount = (
                db
                  .prepare('SELECT COUNT(*) as c FROM decision_traces WHERE project = ?')
                  .get(project) as any
              ).c;
              const intentionsCount = (
                db
                  .prepare('SELECT COUNT(*) as c FROM intentions WHERE project = ?')
                  .get(project) as any
              ).c;
