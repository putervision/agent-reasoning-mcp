...
This project uses `world-model-mcp` with project slug "agent-reasoning-mcp" to maintain a persistent 3D/2D spatial internal world model, entity tracking, object permanence, and movement simulation.
...
This project uses `behavior-mcp` with project slug "agent-reasoning-mcp" to execute deterministic behavior trees at ~60Hz in browser runtimes with reactive triggers and safety guardrails.
...
This project uses `agent-reasoning-mcp` with project slug "agent-reasoning-mcp" to manage goals, decompose complex tasks, evaluate situational trade-offs, and track decision rationale.
...
This project tracks workflow state, tasks, design decisions, and blockers using `state-memory-mcp` with project slug `"agent-reasoning-mcp"`.
